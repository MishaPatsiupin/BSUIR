...
entity als193a is
    Port ( clr : in  STD_LOGIC;
           load : in  STD_LOGIC;  -- активный '0', асинхронный
           up : in  STD_LOGIC;
           down : in  STD_LOGIC;
...
architecture Structural of als193a is
    ...
    signal clk_int : STD_LOGIC;  -- up AND down для rising edge передачи
...
begin
    ...
    -- up_cond (toggle для вверх)
    up_c0 <= '1';
    up_c1 <= qa_int;
    up_c2 <= qa_int and qb_int;
    up_c3 <= qa_int and qb_int and qc_int;
    
    -- down_cond (toggle для вниз)
    down_c0 <= '1';
    down_c1 <= not qa_int;
    down_c2 <= not qa_int and not qb_int;
    down_c3 <= not qa_int and not qb_int and not qc_int;
    
    -- Выбор cond по режиму (down='1' -> up, up='1' -> down)
    cond0 <= up_c0 when down = '1' else down_c0;
    cond1 <= up_c1 when down = '1' else down_c1;
    cond2 <= up_c2 when down = '1' else down_c2;
    cond3 <= up_c3 when down = '1' else down_c3;
    
    -- J/K = cond (toggle для счёта; LOAD игнорируется в FF)
    j0 <= cond0; k0 <= cond0;
    j1 <= cond1; k1 <= cond1;
    j2 <= cond2; k2 <= cond2;
    j3 <= cond3; k3 <= cond3;
    ...
    -- Триггеры
    ff0: jk_ff port map (clk_int, j0, k0, clr, load, a, qa_int);  -- LSB
    ff1: jk_ff port map (clk_int, j1, k1, clr, load, b, qb_int);
    ff2: jk_ff port map (clk_int, j2, k2, clr, load, c, qc_int);
    ff3: jk_ff port map (clk_int, j3, k3, clr, load, d, qd_int);  -- MSB
