...
class MainWindow(QMainWindow):
    # Обновленные таблицы согласно структуре базы данных из бэкапа
    TABLE_KEYS = [
        'Order', 'dish', 'dishingredient', 'employee', 'employeeposition',
        'ingredient', 'orderdish', 'position', 'supplier', 'visitor'
    ]
    
    # Таблицы, требующие админских прав (справочники)
    LOOKUP_KEYS = {'position', 'supplier', 'ingredient', 'dish'}
    ...
    def __init__(self):
        ...
        # Load default table
        self.cmb_tables.setCurrentText('Order')
        self.current_table = 'Order'
    ...
    def show_about(self):
        QMessageBox.about(self, "About", "Restaurant Management System\n\nA modern interface for restaurant database management.")
    ...
    def _find_pk(self, row):
        # Поиск первичного ключа в соответствии со структурой базы
        pk_mapping = {
            'Order': 'orderid',
            'dish': 'dishid',
            'dishingredient': None,  # составной ключ
            'employee': 'employeeid',
            'employeeposition': None,  # составной ключ
            'ingredient': 'ingredientid',
            'orderdish': None,  # составной ключ
            'position': 'positionid',
            'supplier': 'supplierid',
            'visitor': 'visitorid'
        }
        
        if self.current_table in pk_mapping:
            pk = pk_mapping[self.current_table]
            if pk and pk in row:
                return pk
        
        # Резервный поиск по окончанию
        for k in row.keys():
    ...
    def on_add(self):
        def do_add():
            pk_candidates = [c for c in (self.model.columns or []) if c.endswith('id') or c.endswith('_id')]
