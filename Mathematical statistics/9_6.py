import math
import scipy.stats as stats

# Ваша выборка данных
data = [-7.56, -7.49, -7.49, -7.49, -7.36, -7.34, -7.32, -7.12, -7.11, -6.99, -6.95, -6.89, -6.82, -6.78, -6.78, -6.70,
        -6.64, -6.57, -6.51, -6.41, -6.39, -6.30, -6.23, -6.15, -6.15, -6.15, -6.14, -6.04, -6.03, -6.02, -5.83, -5.69,
        -5.64, -5.60, -5.40, -5.37, -5.36, -5.28, -5.05, -4.97, -4.93, -4.85, -4.78, -4.75, -4.37, -4.34, -4.18, -3.98,
        -3.94, -3.94, -3.87, -3.85, -3.78, -3.77, -3.66, -3.64, -3.51, -3.35, -3.31, -3.12, -3.10, -3.03, -2.98, -2.95,
        -2.92, -2.92, -2.87, -2.79, -2.79, -2.77, -2.74, -2.60, -2.59, -2.56, -2.49, -2.48, -2.48, -2.45, -2.44, -2.29,
        -2.26, -2.20, -2.04, -2.01, -1.92, -1.87, -1.85, -1.85, -1.83, -1.57, -1.57, -1.56, -1.51, -1.44, -1.32, -1.29,
        -1.17, -1.05, -0.94, -0.81, -0.77]
n = len(data)
# Вычисление выборочного среднего
sample_mean = sum(data) / len(data)
print("среднее: ", sample_mean)

# Вычисление выборочного среднеквадратического отклонения (стандартного отклонения)
sample_std = math.sqrt(sum((x - sample_mean) ** 2 for x in data) / len(data))
print("ско: ", sample_std)


figna1 = sample_std * sample_mean / 10
print("figna1: ", figna1)

# Интервальная оценка математического ожидания
mean_lower_bound = sample_mean - figna1
mean_upper_bound = sample_mean + figna1
print("Интервальная оценка математического ожидания:")
print("Нижняя граница:", mean_lower_bound)
print("Верхняя граница:", mean_upper_bound)

# Вычисление значения хи-квадрат распределения (chi2) для дисперсии
#chi2_lower = stats.chi2.ppf((1 + confidence_level) / 2, df=n - 1)
#chi2_upper = stats.chi2.ppf((1 - confidence_level) / 2, df=n - 1)



# Интервальная оценка дисперсии
#variance_lower_bound = (n - 1) * sample_std ** 2 / chi2_upper
#variance_upper_bound = (n - 1) * sample_std ** 2 / chi2_lower

# Вывод результатов


#print("Интервальная оценка дисперсии:")
#print("Нижняяграница:", variance_lower_bound)
#print("Верхняя граница:", variance_upper_bound)