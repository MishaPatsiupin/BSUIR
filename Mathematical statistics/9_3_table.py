...
intervals = [(min_value + i * interval_width, min_value + (i + 1) * interval_width) for i in range(k)]
...
for j, interval in enumerate(intervals):
    ...
    Bj = interval[1]
    hj = interval_width
    vj = sum(1 for x in data if Aj <= x < Bj)
    p_j = vj / N
    f_j = p_j / hj

    # Добавление значений в таблицу
    table.append([j, Aj, Bj, hj, vj, p_j, f_j])

# Вывод таблицы
header = ['j', 'Aj', 'Bj', 'hj', 'vj', 'p*j', 'f*j']
print('{:^4s} | {:^9s} | {:^9s} | {:^9s} | {:^3s} | {:^9s} | {:^9s}'.format(*header))
print('-' * 77)
for row in table:
    print('{:^4.0f} | {:9.4f} | {:9.4f} | {:9.4f} | {:^3.0f} | {:9.4f} | {:9.4f}'.format(*row))