...
// -------------------------- Settings --------------------------
const std::string INPUT_FILE = "2.bmp";
...
bool load_bmp(const std::string& filename, std::vector<uchar4>& image, int& width, int& height, int& out_bpp) {
    ...
    if (!file) { std::cerr << "Failed to read BITMAPFILEHEADER\n"; return false; }
    if (fh.bfType != 0x4D42) { std::cerr << "Not a BMP file\n"; return false; }

    // Read biSize
    uint32_t biSize = 0;
    file.read(reinterpret_cast<char*>(&biSize), sizeof(biSize));
    if (!file) { std::cerr << "Failed to read biSize\n"; return false; }
    ...
    if (biSize >= sizeof(BITMAPINFOHEADER)) {
        ...
        if (!file) { std::cerr << "Failed to read BITMAPINFOHEADER\n"; return false; }
        if (biSize > sizeof(BITMAPINFOHEADER)) {
    ...
    if (!file) { std::cerr << "Failed to seek to pixel data\n"; return false; }
    ...
    for (int y = 0; y < height; ++y) {
        ...
        if (!file) { std::cerr << "Failed to read row " << y << std::endl; return false; }
        ...
        if (bpp == 24) {
            for (int x = 0; x < width; ++x) {
                ...
                image[actualY * width + x] = { r, g, b, 255 };
            }
        } else if (bpp == 32) {
            for (int x = 0; x < width; ++x) {
                ...
                unsigned char a = rowbuf[x*4 + 3];
                image[actualY * width + x] = { r, g, b, a };
            }
        } else if (bpp == 8) {
            for (int x = 0; x < width; ++x) {
                ...
                if (idx < palette.size()) {
                    ...
                    image[actualY * width + x] = { q.rgbRed, q.rgbGreen, q.rgbBlue, 255 };
                } else {
                    unsigned char v = idx;
                    image[actualY * width + x] = { v, v, v, 255 };
                }
            }
        } else if (bpp == 4) {
...
// -------------------------- Save BMP (24-bit) --------------------------
bool save_bmp(const std::string& filename, const std::vector<uchar4>& image, int width, int height) {
    BITMAPFILEHEADER fh{};
    BITMAPINFOHEADER ih{};
    int bytesPerPixel = 3;
    ...
    for (int y = height - 1; y >= 0; --y) {
        for (int x = 0; x < width; ++x) {
            const uchar4& p = image[y * width + x];
            row[x*3 + 0] = p.z;
            row[x*3 + 1] = p.y;
            row[x*3 + 2] = p.x;
        }
...
// -------------------------- CPU implementations --------------------------
std::vector<uchar4> gaussian_filter_color_cpu(const std::vector<uchar4>& input, int width, int height, double& cpu_ms) {
    std::vector<uchar4> output(static_cast<size_t>(width) * height);
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            uchar4 r;
            r.x = static_cast<unsigned char>(std::min(std::max(sumx, 0.0f), 255.0f));
            r.y = static_cast<unsigned char>(std::min(std::max(sumy, 0.0f), 255.0f));
            r.z = static_cast<unsigned char>(std::min(std::max(sumz, 0.0f), 255.0f));
            r.w = 255;
            output[y * width + x] = r;
        }
    }
...
// -------------------------- CUDA kernels --------------------------
__device__ uchar4 process_pixel_color(const uchar4* src, int width, int height, int x, int y) {
    ...
    for (int ky = -2; ky <= 2; ky++) {
        for (int kx = -2; kx <= 2; kx++) {
            ...
            uchar4 pixel = src[pos_y * width + pos_x];
    ...
    uchar4 result;
    result.x = static_cast<unsigned char>(min(max(sum.x, 0.0f), 255.0f));
    result.y = static_cast<unsigned char>(min(max(sum.y, 0.0f), 255.0f));
    result.z = static_cast<unsigned char>(min(max(sum.z, 0.0f), 255.0f));
    result.w = 255;
    return result;
}

__global__ void gaussian_filter_color_kernel(const uchar4* input, uchar4* output, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x < width && y < height) {
        output[y * width + x] = process_pixel_color(input, width, height, x, y);
    }
}
...
// -------------------------- GPU wrappers --------------------------
void gaussian_filter_color_gpu(const std::vector<uchar4>& input, std::vector<uchar4>& output, int width, int height, float& gpu_time_ms) {
    ...
    cuda_check(cudaMalloc(&d_input, size), "cudaMalloc d_input color");
    cuda_check(cudaMalloc(&d_output, size), "cudaMalloc d_output color");
    cuda_check(cudaMemcpy(d_input, input.data(), size, cudaMemcpyHostToDevice), "cudaMemcpy H2D color");

    dim3 blockSize(16, 16);
    dim3 gridSize((width + blockSize.x - 1) / blockSize.x, (height + blockSize.y - 1) / blockSize.y);

    cudaEventRecord(start);
    gaussian_filter_color_kernel<<<gridSize, blockSize>>>(d_input, d_output, width, height);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&gpu_time_ms, start, stop);

    cuda_check(cudaMemcpy(output.data(), d_output, size, cudaMemcpyDeviceToHost), "cudaMemcpy D2H color");
...
// -------------------------- Autodetect mode --------------------------
int detect_image_mode(const std::vector<uchar4>& img, int width, int height, int header_bpp) {
    ...
    for (size_t i = 0; i < total; i += step) {
        const uchar4& p = img[i];
        if (p.x == p.y && p.y == p.z) ++gray_count;
        ++checked;
    }
...
// -------------------------- Comparison --------------------------
void compare_color_images(const std::vector<uchar4>& a, const std::vector<uchar4>& b, int width, int height) {
    size_t total = static_cast<size_t>(width) * height;
    size_t mismatches = 0;
    ...
    if (mismatches && first_idx != SIZE_MAX) {
        ...
        std::cout << "CPU pixel: R=" << int(a[first_idx].x) << " G=" << int(a[first_idx].y) << " B=" << int(a[first_idx].z) << std::endl;
        std::cout << "GPU pixel: R=" << int(b[first_idx].x) << " G=" << int(b[first_idx].y) << " B=" << int(b[first_idx].z) << std::endl;
    }
}
...
int main() {
    ...
    */
    ...
    if (mode == 1) {
        // Color path
        std::vector<uchar4> cpu_out;
        std::vector<uchar4> gpu_out(static_cast<size_t>(width) * height);
        double cpu_ms = 0.0;
        float gpu_ms = 0.0f;
        ...
        for (int i = 0; i < width * height; ++i) {
            const uchar4& p = input_image[i];
            gray_input[i] = static_cast<unsigned char>(0.299f * p.x + 0.587f * p.y + 0.114f * p.z);
        }
