...
;* LAB6.ASM *
;* *
;* Этот код на языке ассемблера является частью лабораторной работы по курсу *
;* программирования на языке ассемблера. Он демонстрирует различные концепции, *
;* такие как реальный режим, защищенный режим, обработка прерываний и сегментные *
;* регистры. *
;* *
;* Код инициализирует систему, настраивает Глобальную Таблицу Дескрипторов (GDT), *
;* Таблицу Дескрипторов Прерываний (IDT) и подготавливает систему к переключению *
;* из реального режима в защищенный режим. Он также включает обработчики прерываний *
;* для таймера и клавиатуры, а также заполнители для других обработчиков прерываний.*
;* *
;* Примечание: Этот код предоставляется только для образовательных целей и может *
;* не подходить для производственных сред. *
;**************************************************************************************
...
START:
    ...
    mov ah,7h
    int 21h                                    
PREPARE_RTC:                                    ;Prepare RTC clock
    mov al,0Bh
    out 70h,al                                  ;Select register 0Bh
    in  al,71h                                  
    or  al,00000100b                            ;Set DM bit to 1 - binary representation of time
    out 71h,al                                  
ENABLE_A20:                                     ;Enable A20 line (to get more address memory)
    in  al,92h                                                                              
    or  al,2                                    ;Set bit 1 to 1                                                   
    out 92h,al           
SAVE_MASK:                                      ;Save interrupt masks     
    in      al,21h
    mov     INT_MASK_M,al                  
    in      al,0A1h
    mov     INT_MASK_S,al                 
DISABLE_INTERRUPTS:                             ;Disable maskable and non-maskable interrupts        
    cli                                         ;Disable maskable interrupts
    in  al,70h	
	or	al,10000000b                            ;Set bit 7 to 1 to disable non-maskable interrupts
	out	70h,al
	nop	                                        ;skip clock cycle
LOAD_GDT:                                       ;Load Global Descriptor Table (GDT)            
    mov ax,DATA
    mov dl,ah
    xor dh,dh
    shl ax,4
    ...
    mov si,ax
    mov di,dx
WRITE_GDT:                                      ;Write GDT descriptor
    lea bx,GDT_GDT
    mov ax,si
    mov dx,di
    add ax,offset GDT
...
WRITE_CODE_RM:                                  ;Write real mode code segment descriptor
    lea bx,GDT_CODE_RM
    mov ax,cs
    xor dh,dh
    mov dl,ah
    ...
    mov [bx][S_DESC.BASE_M],dl
    mov [bx][S_DESC.BASE_H],dh
WRITE_DATA:                                     ;Write data segment descriptor
    lea bx,GDT_DATA
    mov ax,si
    mov dx,di
    mov [bx][S_DESC.BASE_L],ax
    mov [bx][S_DESC.BASE_M],dl
    mov [bx][S_DESC.BASE_H],dh
WRITE_STACK:                                    ;Write stack segment descriptor
    lea bx, GDT_STACK
    mov ax,ss
    xor dh,dh
    mov dl,ah
    ...
    mov [bx][S_DESC.BASE_M],dl
    mov [bx][S_DESC.BASE_H],dh
WRITE_CODE_PM:                                  ;Write protected mode code descriptor
    lea bx,GDT_CODE_PM
    mov ax,CODE_PM
    xor dh,dh
    mov dl,ah
    ...
    mov [bx][S_DESC.BASE_M],dl
    mov [bx][S_DESC.BASE_H],dh        
WRITE_IDT:                                      ;Write Interrupt Descriptor Table (IDT) descriptor
    lea bx,GDT_IDT
    mov ax,si
    mov dx,di
    add ax,OFFSET IDT
    ...
    mov IDTR.IDT_L,ax
    mov IDTR.IDT_H,dx
FILL_IDT:                                       ;Fill Interrupt Descriptor Table (IDT)
    irpc    N, 0123456789ABCDEF                 ;Fill vectors 00-0F with exceptions
        lea eax, EXC_0&N
        mov IDT_0&N.OFFS_L,ax
        shr eax, 16
        mov IDT_0&N.OFFS_H,ax
    endm
    irpc    N, 0123456789ABCDEF                 ;Fill vectors 10-1F with exceptions
        lea eax, EXC_1&N
    ...
    lea eax, TIMER_HANDLER                      ;Place timer interrupt handler at gate 20h
    mov IDT_TIMER.OFFS_L,ax
    shr eax, 16
    mov IDT_TIMER.OFFS_H,ax

    lea eax, KEYBOARD_HANDLER                   ;Place keyboard interrupt handler at gate 21h
    mov IDT_KEYBOARD.OFFS_L,ax
    shr eax, 16
    mov IDT_KEYBOARD.OFFS_H,ax

    irpc    N, 234567                           ;Fill vectors 22-27 with placeholders
    ...
    irpc    N, 89ABCDEF                         ;Fill vectors 28-2F with placeholders
        lea eax,IDLE_IRQ_SLAVE
        mov IDT_2&N.OFFS_L,ax
        shr eax,16
        mov IDT_2&N.OFFS_H,ax
    endm
    lgdt fword ptr GDT_GDT                      ;Load GDTR register
    lidt fword ptr IDTR                         ;Load IDTR register
    mov eax,cr0                                 ;Get control register cr0
    or  al,00000001b                            ;Set PE bit to 1
    mov cr0,eax                                 ;Write modified cr0 to enable protected mode
OVERLOAD_CS:                                    ;Reload code segment with its descriptor
    db  0eah                                    ;Artificially constructed far jump instruction
    dw  offset OVERLOAD_SEGMENT_REGISTERS       ;Will change the contents of IP and CS to the descriptor selector
    dw  CODE_RM_DESC        
OVERLOAD_SEGMENT_REGISTERS:                     ;Reinitialize other segment registers with descriptors
    mov ax,DATA_DESC                            
    mov ds,ax                                   ;DS contains the data segment descriptor selector
    mov es,ax                                   ;Same as ES
    mov ax,STACK_DESC                           
    mov ss,ax                                   ;SS contains the stack segment descriptor selector
    xor ax,ax
    mov fs,ax                                   ;Clear FS register
    mov gs,ax                                   ;Clear GS register
    lldt ax                                     ;Clear LDTR register - do not use local descriptor tables
PREPARE_TO_RETURN:                              ;Prepare for return to real mode
    push cs                                     ;Save code segment in stack
    push offset BACK_TO_RM                      ;Save return point offset
    lea  edi,ENTER_PM                           ;Get entry point to protected mode
    mov  eax,CODE_PM_DESC                       ;Get protected mode code descriptor
    push eax                                    ;Push them onto the stack
    push edi                                    
REINITIALIAZE_CONTROLLER_FOR_PM:                ;Reinitialize interrupt controller to vectors 20h, 28h
    mov al,00010001b                            ;ICW1 - reinitialize interrupt controller
    out 20h,al                                  ;Reinitialize master controller
    out 0A0h,al                                 ;Reinitialize slave controller
    mov al,20h                                  ;ICW2 - base interrupt vector number
    out 21h,al                                  ;for master controller
    mov al,28h                                  ;ICW2 - base interrupt vector number
    out 0A1h,al                                 ;for slave controller
    mov al,04h                                  ;ICW3 - master controller connected to line 3
    out 21h,al       
    mov al,02h                                  ;ICW3 - slave controller connected to line 3
    out 0A1h,al      
    mov al,11h                                  ;ICW4 - special fully nested mode for master controller
    out 21h,al        
    mov al,01h                                  ;ICW4 - normal fully nested mode for slave controller
    out 0A1h,al       
    mov al, 0                                   ;Unmask interrupts
    out 21h,al                                  ;for master controller
    out 0A1h,al                                 ;for slave controller
ENABLE_INTERRUPTS_0:                            ;Enable maskable and non-maskable interrupts
    in  al,70h	
	and	al,01111111b                            ;Set bit 7 to 0 to enable non-maskable interrupts
	out	70h,al
	nop
    sti                                         ;Enable maskable interrupts
GO_TO_CODE_PM:                                  ;Jump to protected mode code segment
    db 66h                                      ;next operation is 32-bit operation
    retf
    
BACK_TO_RM:                                     ;Return point to real mode
    cli                                         ;Disable maskable interrupts
    in  al,70h	                                ;and non-maskable interrupts
	or	AL,10000000b                            ;Set bit 7 to 1 to disable non-maskable interrupts
	out	70h,AL
	nop
