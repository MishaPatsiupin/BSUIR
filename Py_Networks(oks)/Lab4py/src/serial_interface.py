...
class SerialInterfaceApp:
    ...
    def setup_ui(self):
        ...
        self.left_output_text = scrolledtext.ScrolledText(self.left_output_frame, width=100, height=20, state='disabled')  # Увеличиваем ширину и высоту
        ...
        self.right_output_text = scrolledtext.ScrolledText(self.right_output_frame, width=100, height=20, state='disabled')  # Увеличиваем ширину и высоту
...
#-------------------------------------------------------------------------        
        ...
        while self.logic.simulate_channel_busy():
            output_text.config(state='normal')
            output_text.insert(tk.END, "Канал занят. Повторная попытка...\n")
            output_text.config(state='disabled')
            output_text.update_idletasks()
            time.sleep(0.1) 
        
        while counter_try < MAX_RETRIES:

            response = self.logic.send_packet(connected_port, selected_port, packet, parity)

            time.sleep(0.1)  # Окно коллизий

            if self.logic.simulate_collision():
                output_text.config(state='normal')
                output_text.insert(tk.END, "+\n")
                output_text.insert(tk.END, "Jam-signal\n")
                output_text.config(state='disabled')
                output_text.update_idletasks()
                counter_try += 1
                time.sleep(self.logic.calculate_random_delay(counter_try, MAX_RETRIES))
                #response = self.logic.send_packet(connected_port, selected_port, packet, parity)
            else:
                output_text.config(state='normal')
                output_text.insert(tk.END, "-\n")
            ...
            if counter_try == MAX_RETRIES:
                output_text.config(state='normal')
                output_text.insert(tk.END, "Превышено максимальное количество попыток.\n")
                output_text.config(state='disabled')
                output_text.update_idletasks()
                return
