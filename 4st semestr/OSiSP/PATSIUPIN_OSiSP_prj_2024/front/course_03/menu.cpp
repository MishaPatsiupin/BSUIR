...
void process_command_line_options(int argc, char* argv[]) {
    ...
    desc.add_options()
            ...
            ("access-permissions", po::value<std::string>(), "Specify the access permissions (xxx), 1<=x<=7, or 000 for no encryption")
    ...
    } else if (vm.count("access-permissions")) {
        set_access_permissions(vm);
...
void configure_protection() {
    ...
    do {
        ...
        switch (config_choice) {
            ...
            case 5: {
                std::cout << "Enter the access permissions (xxx): ";
                std::string buffer;
                std::cin >> buffer;
