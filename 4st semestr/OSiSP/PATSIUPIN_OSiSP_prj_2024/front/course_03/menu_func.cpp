...
void set_access_permissions(po::variables_map& vm) {
    std::string buffer = vm["access-permissions"].as<std::string>();
    if (buffer.size() == 3 && std::all_of(buffer.begin(), buffer.end(), [](char c){ return c >= '1' && c <= '7'; }) or buffer == "000") {
        flagMenu.access_permissions = buffer;
        flagMenu.flag_access_permissions = 1;
        std::cout << "Access Permissions: " << flagMenu.access_permissions << "\n";
    } else {
        std::cout << "Invalid input. Please enter a three-digit number, each digit between 1 and 7.\n";
    }
    ignore_cin();
}
...
std::cout << "* Access Permissions: " << bufferMenu.access_permissions << std::endl;
...
void show_help() {
    ...
    std::cout << "** Specify the access permissions: Specify the access permissions for the file or       *\n";
    std::cout << "**                                 directory (xxx), where each x is a digit between 1   *\n";
    std::cout << "**                                 and 7, or 000 - encryption                           *\n";
